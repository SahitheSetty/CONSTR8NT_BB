...
async def test_replay_mode_raises_named_error_for_a_probe_the_adapter_cannot_answer(
    p1_raw, p2_analysis
):
    # dns_consistency has no adapter mapping; a spec that makes it the only
    # informative probe forces the engine to select it, and ReplayMock must
    # then fail loudly rather than inventing a symbol.
    spec = _spec_over(["dns_consistency"])

    with pytest.raises(KeyError, match="dns_consistency"):
        await diagnose_from_upstream(p1_raw, p2_analysis, mode="replay", spec=spec)
